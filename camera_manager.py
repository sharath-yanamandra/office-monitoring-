...
class CameraReader:
    ...
    def __init__(self, camera_id: str, stream_url: str, frame_queue: queue.Queue, result_callback=None, 
        ...
        self.target_fps = target_fps if target_fps is not None else Config.READER_FPS_LIMIT
        ...
        self.enable_motion_detection = Config.MOTION_DETECTION_ENABLED
    ...
    def _detect_motion(self, frame):
        ...
        #     if self.motion_warmup_frames < Config.MOTION_WARMUP_FRAMES:
        ...
        try:
            ...
            # Skip warmup frames to let background model stabilize
            self.motion_warmup_frames += 1
            warmup_frames = getattr(Config, 'MOTION_WARMUP_FRAMES', 30)
            if self.motion_warmup_frames < warmup_frames:
                return True  # Allow processing during warmup
            ...
            motion_detected = motion_ratio > Config.MOTION_AREA_THRESHOLD
    ...
    # camera_manager.py - PRECISE FIX for _reader_worker method

    def _reader_worker(self):
        """Camera reader worker thread with robust frame handling - FIXED VERSION"""
        ...
        while self.running:
            try:
                ...
                while self.running and self.is_connected:
                    ...
                    # =================================================================================
                    # CRITICAL FIX: Robust frame reading to handle OpenCV version differences
                    # =================================================================================
                    try:
                        read_start = time.time()
                        cap_result = self.cap.read()
                        read_time = time.time() - read_start

                        # Handle different OpenCV versions and return types robustly
                        ret = False
                        frame = None
                        
                        if cap_result is None:
                            # Some OpenCV versions return None on failure
                            ret, frame = False, None
                            self.logger.debug(f"cap.read() returned None for camera {self.camera_id}")
                        elif isinstance(cap_result, tuple):
                            if len(cap_result) == 2:
                                # Standard case: (ret, frame)
                                ret, frame = cap_result
                            elif len(cap_result) == 1:
                                # Some versions return single element tuple
                                ret = bool(cap_result[0]) if cap_result[0] is not None else False
                                frame = cap_result[0] if ret and isinstance(cap_result[0], np.ndarray) else None
                            elif len(cap_result) > 2:
                                # Handle unexpected tuple sizes gracefully
                                ret = bool(cap_result[0]) if len(cap_result) > 0 and cap_result[0] is not None else False
                                frame = cap_result[1] if len(cap_result) > 1 and isinstance(cap_result[1], np.ndarray) else None
                                self.logger.warning(f"Unexpected cap.read() return tuple size: {len(cap_result)} for camera {self.camera_id}")
                            else:
                                # Empty tuple
                                ret, frame = False, None
                        elif isinstance(cap_result, np.ndarray):
                            # Direct frame return (some camera interfaces)
                            ret, frame = True, cap_result
                        elif isinstance(cap_result, bool):
                            # Just boolean return
                            ret, frame = cap_result, None
                        else:
                            # Unknown return type
                            ret, frame = False, None
                            self.logger.warning(f"Unknown cap.read() return type: {type(cap_result)} for camera {self.camera_id}")
                        ...
                        if not isinstance(frame, np.ndarray):
                            self.logger.warning(f"Invalid frame type from camera {self.camera_id}: {type(frame)}")
                            self.frames_dropped += 1
                            frames_dropped += 1
                            continue
                            
                        if frame.size == 0:
                            self.logger.warning(f"Empty frame from camera {self.camera_id}")
                            self.frames_dropped += 1
                            frames_dropped += 1
                            continue
                            
                        if len(frame.shape) != 3 or frame.shape[2] != 3:
                            self.logger.warning(f"Invalid frame dimensions from camera {self.camera_id}: {frame.shape}")
                            self.frames_dropped += 1
                            frames_dropped += 1
                            continue
                    ...
                    except Exception as frame_read_error:
                        self.logger.error(f"Error reading frame from camera {self.camera_id}: {str(frame_read_error)}")
                        self.frames_dropped += 1
                        frames_dropped += 1
                    ...
                    try:
                        ...
                        if self.video_buffer_callback:
                            try:
                                ...
                                self.logger.error(f"Error in video buffer callback for camera {self.camera_id}: {str(callback_error)}")
                            
                    except Exception as e:
                        self.logger.error(f"Error processing frame buffer for camera {self.camera_id}: {str(e)}")
                    ...
                    if time_since_last_processing >= processing_interval:
                        ...
                        if self.motion_detected and not motion_detected:
                            ...
                            if time_since_motion > Config.MOTION_IDLE_TIMEOUT:
                                self.motion_detected = False
                                self.motion_stats['idle_periods'] += 1
                                self.logger.info(f"Camera {self.camera_id}: No motion for {Config.MOTION_IDLE_TIMEOUT}s, "
    ...
    # Additional helper method to add to CameraReader class
    def _validate_frame(self, frame):
        ...
        if frame.size == 0:
            return False
        if len(frame.shape) != 3:  # Should be height, width, channels
            return False
        if frame.shape[2] != 3:  # Should be BGR/RGB
            return False
        return True
    # def _reader_worker(self):
    ...
    #                         ret, frame = result
    #                     else:
    #                         ret = result[0] if len(result) > 0 else False
    #                         frame = result[1] if len(result) > 1 else None
    #                 else:
    #                     ret, frame = False, None
        ...
        try:
            ...
            try:
                ...
                if fps > 0:
                    ...
                    new_buffer_size = int(Config.VIDEO_BUFFER_PAST_SECONDS * fps)
                    self.video_buffer = queue.Queue(maxsize=new_buffer_size)
                    
                    self.logger.info(f"Updated video buffer size to {new_buffer_size} frames for {Config.VIDEO_BUFFER_PAST_SECONDS}s past footage at {fps:.2f} FPS")
...
class DatacenterCameraManager:
    ...
    def __init__(self):
        ...
        self.logger.info(f"- Reader FPS limit: {Config.READER_FPS_LIMIT}")
        self.logger.info(f"- Batch size: {Config.BATCH_SIZE}")
        self.logger.info(f"- Max queue size: {Config.MAX_QUEUE_SIZE}")
        self.logger.info(f"- Activity levels - High: {Config.ACTIVITY_LEVEL_HIGH}, Medium: {Config.ACTIVITY_LEVEL_MEDIUM}, Low: {Config.ACTIVITY_LEVEL_LOW}")
        ...
        self.num_batch_threads = Config.NUM_BATCH_THREADS if hasattr(Config, 'NUM_BATCH_THREADS') else 2
    ...
    def is_camera_active(self, camera_id: str) -> bool:
        ...
        try:
            ...
            # FIX: Check the actual attribute name in your camera manager
            # Look for these possible attribute names:
            if hasattr(self, 'camera_readers'):
                camera_dict = self.camera_readers
            elif hasattr(self, 'readers'):
                camera_dict = self.readers
            elif hasattr(self, 'camera_instances'):
                camera_dict = self.camera_instances
            elif hasattr(self, 'active_cameras'):
                camera_dict = self.active_cameras
            else:
                # Fallback - return True if camera was started
                return True
            
            return (camera_id_str in camera_dict and 
                    hasattr(camera_dict[camera_id_str], 'running') and
                    camera_dict[camera_id_str].running)
                    
        except Exception as e:
            self.logger.error(f"Error checking camera {camera_id} status: {e}")
            return False
    ...
    def get_camera_status(self, camera_id: str) -> dict:
        ...
        try:
            ...
            if camera_id_str not in camera_dict:
                return {'status': 'not_found', 'active': False}
            
            camera = camera_dict[camera_id_str]
            return {
                'status': 'active' if getattr(camera, 'running', True) else 'inactive',
                'active': getattr(camera, 'running', True),
                'frames_captured': getattr(camera, 'frames_captured', 0),
                'is_connected': getattr(camera, 'is_connected', False)
            }
        except Exception as e:
            self.logger.error(f"Error getting camera {camera_id} status: {e}")
    ...
    #     try:
    #         camera_id_str = str(camera_id)
    #         return (camera_id_str in self.cameras and 
    #                 hasattr(self.cameras[camera_id_str], 'running') and
    #                 self.cameras[camera_id_str].running)
    #     except Exception as e:
    #         self.logger.error(f"Error checking camera {camera_id} status: {e}")
    #         return False
    ...
    #     try:
    #         camera_id_str = str(camera_id)
    #         if camera_id_str not in self.cameras:
    #             return {'status': 'not_found', 'active': False}
            
    #         camera = self.cameras[camera_id_str]
    #         return {
    ...
    def start_camera(self, camera_id: str, stream_url: str, activity_level: str = 'medium'):
        ...
        camera_queue = queue.Queue(maxsize=Config.MAX_QUEUE_SIZE)
    ...
    def _get_fps_for_activity_level(self, activity_level: str) -> int:
        ...
        if activity_level == 'high':
            return Config.ACTIVITY_LEVEL_HIGH  # High activity areas get 10 FPS
        elif activity_level == 'low':
            return Config.ACTIVITY_LEVEL_LOW  # Low activity areas get 2 FPS
        else:  # medium or any other value
            return Config.ACTIVITY_LEVEL_MEDIUM  # Medium activity gets 4 FPS
    ...
    def _batch_collection_worker(self, worker_id: int):
        ...
        while self.batch_collection_running:
            try:
                ...
                timeout_occurred = (current_time - last_frame_time) >= Config.BATCH_TIMEOUT
                
                if len(pending_frames) >= Config.BATCH_SIZE or (pending_frames and timeout_occurred):
                    # Create a batch from pending frames
                    batch_size = min(Config.BATCH_SIZE, len(pending_frames))
    ...
    def _record_event_video(self, recording_task):
        ...
        try:
            ...
            if recording_task.get('completion_callback') and success:
                try:
                    recording_task['completion_callback'](event_id, video_path, len(all_frames)/Config.VIDEO_FPS)
                except Exception as e:
