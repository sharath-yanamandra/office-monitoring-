...
class DatacenterModelManager:
    ...
    def _initialize_models(self):
        ...
        self._load_model_type('detection', DatacenterConfig.DETECTION_MODEL_PATH)
        
        # Load PPE detection if available
        if hasattr(DatacenterConfig, 'PPE_DETECTION_MODEL_PATH') and DatacenterConfig.PPE_DETECTION_MODEL_PATH:
            self._load_model_type('ppe_detection', DatacenterConfig.PPE_DETECTION_MODEL_PATH)
        
        # Load pose estimation if enabled
        if DatacenterConfig.POSE_ENABLED and hasattr(DatacenterConfig, 'POSE_ESTIMATION_MODEL_PATH'):
            self._load_model_type('pose_estimation', DatacenterConfig.POSE_ESTIMATION_MODEL_PATH)
    ...
    def initialize_models(self):
        ...
        try:
            ...
            with torch.cuda.device(0) if torch.cuda.is_available() else torch.device('cpu'):
                # Detection model
                detection_model = YOLO(DatacenterConfig.DETECTION_MODEL_PATH)
                ...
                if hasattr(DatacenterConfig, 'PPE_DETECTION_MODEL_PATH') and DatacenterConfig.PPE_DETECTION_MODEL_PATH:
                    try:
                        ppe_model = YOLO(DatacenterConfig.PPE_DETECTION_MODEL_PATH)
                        ppe_model.to(device)
                ...
                if DatacenterConfig.POSE_ENABLED:
                    try:
                        pose_estimation_model = YOLO(DatacenterConfig.POSE_ESTIMATION_MODEL_PATH)
